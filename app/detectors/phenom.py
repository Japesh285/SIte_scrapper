...
async def detect_phenom(
    ...
    # Try to extract Phenom API URL from probe result (avoids a separate browser session)
    if probe_result is not None:
        for resp in getattr(probe_result, "responses", []):
            try:
                items = _find_job_items(resp.body)
                if len(items) >= 5:
                    logger.info("[Phenom] API found in probe result: %s (%d jobs)", resp.url, len(items))
                    return {
                        "matched": True,
                        "api_url": resp.url,
